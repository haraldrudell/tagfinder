...
function decomposeHtml(html) {
	...
	for (;;) {
		...
		htmlTag = !tagData.voidElement ? tagData.tag : ''
	...
	function parseClassNames() {
		...
		var match
		var classValue = tagData.attributes['class']
		if (classValue) match = classValue.match(classRegExp)
		if (match) tagData.classes = match
	}
}